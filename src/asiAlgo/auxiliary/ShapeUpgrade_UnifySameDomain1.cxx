                                        ...
                                        TopTools_MapOfShape&            theEdgesMap)
{
  ...
  for (Standard_Integer ind = 1; ind <= theEdges.Length(); ind++)
  {
    ...
    Standard_Real pf,pl;
    const Handle(Geom2d_Curve) PC = BRep_Tool::CurveOnSurface(anEdge,theRefFace,pf,pl);
    //
    if ( PC.IsNull() )
      return false;
  ...
  if (IsUnionOfCirclesPossible)
  {
    ...
    if (V[0].IsSame(V[1])) {
      ...
      if(fabs(lpar)>Precision::Confusion()) {
        ...
        // check orientation
        gp_Dir C_XAxis = C->XAxis().Direction();
        const double ang = C_XAxis.Angle(D2);
        //
        if ( Abs(ang) > Precision::Angular() && Abs(ang - M_PI) > Precision::Angular() )
        {
          gp_Dir ND = C_XAxis.Crossed(D2);
          if ( ND.IsOpposite( C->Axis().Direction(), Precision::Confusion() ) )
          {
            lpar = -lpar;
          }
...
//=======================================================================
//function : CheckClosedContours
//purpose  : Checks if all faces of the passed shape have closed contours.
//=======================================================================

static bool CheckClosedContours(const TopoDS_Shape& shape)
{
  ...
  for ( TopExp_Explorer exp(shape, TopAbs_FACE); exp.More(); exp.Next() )
  {
    ...
    double locTolerance = checker.MaxTolerance(face)*5.0;
    ...
    if (faces.Length() > 1)
    {
      ...
      if (Uperiod != 0.)
      {
        ...
        if (!SeamFound)
        {
          ...
          for (Standard_Integer ii = 1; ii <= edges.Length(); ii++)
          {
            const Handle(Geom2d_Curve)* aPCurvePtr = EdgeNewPCurve.Seek(edges(ii));
            //
            if ( aPCurvePtr == nullptr )
            {
              myFailed = true;
              return Standard_False;
            }

            const Handle(Geom2d_Curve)& aPCurve = *aPCurvePtr;
      ...
      for (Standard_Integer ii = 1; ii <= edges.Length(); ii++)
      {
        ...
        Standard_Real pf,pl;
        const Handle(Geom2d_Curve) PC = BRep_Tool::CurveOnSurface(anEdge,F_RefFace,pf,pl);
        //
        if ( PC.IsNull() )
          return false;
            ...
            if (CurVertex.IsSame(StartVertex))
            {
                  ...
                  Abs(StartPoint.X() - CurPoint.X()) > Uperiod/2) //end of parametric space
              {
                ...
                ReconstructMissedSeam(edges, UsedEdges, F_RefFace, CurVertex,
                                      ...
                                      StartOfNextEdge, LastVertexOfSeam, VEmap, myFailed);
              }
              else
              {
                break; //end of wire
...
bool ShapeUpgrade_UnifySameDomain1::Build() 
{
  ...
  bool wasValid = false;
  {
    ...
    //
    wasValid = checker.CheckBasic(myShape);
  }
#endif
  ...
  if ( wasValid )
  {
    asiAlgo_CheckValidity checker(m_progress, m_plotter);
    //
    if ( !checker.CheckBasic(myShape) )
    {
      m_progress.SendLogMessage(LogErr(Normal) << "Invalid shape after face maximization.");
      numInvalid++;
      std::cout << "numInvalid: " << numInvalid << std::endl;
    }
  }
#endif
