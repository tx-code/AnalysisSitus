#ifndef asiAlgo_QuickHull_h
#define asiAlgo_QuickHull_h
  ...
  public:
    ...
    struct Face {
      ...
      /*Face(const Face& face):
        m_he(face.m_he),
        m_P(face.m_P),
        m_mostDistantPointDist(face.m_mostDistantPointDist),
        m_mostDistantPoint(face.m_mostDistantPoint),
        m_visibilityCheckedOnIteration(face.m_visibilityCheckedOnIteration),
        m_isVisibleFaceOnCurrentIteration(face.m_isVisibleFaceOnCurrentIteration),
        m_inFaceStack(face.m_inFaceStack),
        m_horizonEdgesOnCurrentIteration(face.m_horizonEdgesOnCurrentIteration),
        m_pointsOnPositiveSide(std::shared_ptr<std::vector<IndexType>>(new std::vector<IndexType>(*face.m_pointsOnPositiveSide)))
        //m_pointsOnPositiveSide(new std::vector<IndexType>(*face.m_pointsOnPositiveSide))
      {
      }*/
      ...
      /*Face& operator=(const Face& face)
      {
        ...
        this->m_he = face.m_he;
        this->m_P = face.m_P;
        this->m_mostDistantPointDist = face.m_mostDistantPointDist;
        this->m_mostDistantPoint = face.m_mostDistantPoint;
        this->m_visibilityCheckedOnIteration = face.m_visibilityCheckedOnIteration;
        this->m_isVisibleFaceOnCurrentIteration = face.m_isVisibleFaceOnCurrentIteration;
        this->m_inFaceStack = face.m_inFaceStack;
        this->m_horizonEdgesOnCurrentIteration = face.m_horizonEdgesOnCurrentIteration;
        this->m_pointsOnPositiveSide.reset(new std::vector<IndexType>(*face.m_pointsOnPositiveSide));
        return *this;
      }*/
    };
    ...
    ConvexHull(const MeshBuilder<T>& mesh, const VertexDataSource<T>& pointCloud, bool CCW, bool useOriginalIndices) {
      ...
      while (faceStack.size()) {
        ...
        if (faceProcessed[top]) {
          ...
          if (!useOriginalIndices) {
            for (auto& v : vertices) {
              auto it1 = vertexIndexMapping.find(v);
              if (it1 == vertexIndexMapping.end()) {
