...
namespace
{
    ...
    void makeGeometryCompatibleImpl(std::vector<SplineAdapter>& splines_vector, double par_tolerance)
    {
        ...
        if (!haveSameRange(splines_vector, par_tolerance)) {
            throw Standard_ProgramError("B-splines don't have the same parameter range at least in one direction (u / v) in method createCommonKnotsVectorImpl!");
        }

        // all B-spline splines must have the same degree in the chosen direction
        if (!haveSameDegree(splines_vector)) {
            throw Standard_ProgramError("B-splines don't have the same degree at least in one direction (u / v) in method createCommonKnotsVectorImpl!");
        }
        ...
        for (typename std::vector<SplineAdapter>::iterator splineIt = splines_vector.begin()+1; splineIt != splines_vector.end(); ++splineIt) {
            ...
            if (spline.getNKnots() != firstSpline.getNKnots()) {
                throw Standard_ProgramError("Unexpected error in Algorithm makeGeometryCompatibleImpl.\nPlease contact the developers.");
            }
        }
...
{
    if (params.size() < 2) {
        throw Standard_ProgramError("Parameters must contain two or more elements.");
    }
...
{
    if ( umax <= umin ) {
        throw Standard_ProgramError("The specified start parameter is larger than the specified end parameter");
    }
                                                                                 ...
                                                                                 size_t n_control_pnts)
{
    if (old_parameters.size() != new_parameters.size()) {
        throw Standard_ProgramError("parameter sizes dont match");
    }
    ...
    if (!interpolationObject.IsDone()) {
        throw Standard_ProgramError("Cannot reparametrize");
    }
...
{
    if (spline.IsNull()) {
        throw Standard_ProgramError("Null Pointer curve");
    }
                                                               ...
                                                               bool parByLength, double tolerance)
{
    ...
    if (curves.size() == 0) {
        std::cout << "Empty curve vector in CTiglBSplineAlgorithms::concatCurves";
        return nullptr;
    }
    ...
    for (unsigned int icurve = 1; icurve < curves.size(); ++icurve) {
        ...
        if (p1.Distance(p2) > tolerance) {
            // error
            std::cout << "Curves not connected within tolerance in concatCurves";
            return nullptr;
        }
    }
                                                                       ...
                                                                       double space_tol)
{
    ...
    if (leftCornerDist > space_tol || rightCornerDist > space_tol) {
        throw Standard_ProgramError("Surfaces don't match within tolerances in CTiglBSplineAlgorithms::concatSurfacesUDir.");
    }
