...
asiAlgo_EXPORT CTiglCurvesToSurface::CTiglCurvesToSurface(std::vector<Handle(Geom_Curve) > const& splines_vector,
...
asiAlgo_EXPORT void CTiglCurvesToSurface::SetMaxDegree(int degree)
{
    assert( degree > 0 );
    _maxDegree = degree;
    Invalidate();
...
asiAlgo_EXPORT void CTiglCurvesToSurface::CalculateParameters(std::vector<Handle(Geom_BSplineCurve)> const& splines_vector)
{
...
asiAlgo_EXPORT void CTiglCurvesToSurface::Perform()
{
    // check amount of given parameters
    if (_parameters.size() != _inputCurves.size()) {
        throw Standard_ProgramError("The amount of given parameters has to be equal to the amount of given B-splines!");
    }
