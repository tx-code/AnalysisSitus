...
public:
     ...
     */
    asiAlgo_EXPORT explicit CTiglCurvesToSurface(const std::vector<Handle(Geom_Curve) >& splines_vector,
     ...
     */
    asiAlgo_EXPORT void SetMaxDegree(int degree);

    /**
     * @brief returns the parameters at the profile curves
     */
    asiAlgo_EXPORT std::vector<double> GetParameters() const
    {
        return _parameters;
    }
...
private:
    asiAlgo_EXPORT void CalculateParameters(std::vector<Handle(Geom_BSplineCurve)> const& splines_vector);
    asiAlgo_EXPORT void Perform();
    asiAlgo_EXPORT void Invalidate() { _hasPerformed = false; }
