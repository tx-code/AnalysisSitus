...
public:
    ...
    asiAlgo_EXPORT static const double REL_TOL_CLOSED;
     ...
     */
    asiAlgo_EXPORT static std::vector<double> computeParamsBSplineCurve(const Handle(TColgp_HArray1OfPnt)& points, double alpha=0.5);
     ...
     */
    asiAlgo_EXPORT static std::vector<double> computeParamsBSplineCurve(const Handle(TColgp_HArray1OfPnt)& points, double umin, double umax, double alpha=0.5);
     ...
     */
    asiAlgo_EXPORT static math_Matrix bsplineBasisMat(int degree, const TColStd_Array1OfReal& flatKnots, const TColStd_Array1OfReal& params, unsigned int derivOrder=0);
     ...
     */
    asiAlgo_EXPORT static std::pair<std::vector<double>, std::vector<double> >
     ...
     */
    asiAlgo_EXPORT static void matchParameterRange(const std::vector<Handle(Geom_BSplineCurve) >& bsplines, double tolerance=1e-15);
     ...
     * @param bsplines The splines to be matched (in/out)
     */
    asiAlgo_EXPORT static void matchDegree(const std::vector<Handle(Geom_BSplineCurve) >& bsplines);
     ...
     */
    asiAlgo_EXPORT static std::vector<Handle(Geom_BSplineCurve)> createCommonKnotsVectorCurve(const std::vector<Handle(Geom_BSplineCurve)>& splines_vector, double tol);
     ...
     */
    asiAlgo_EXPORT static std::vector<Handle(Geom_BSplineSurface) > createCommonKnotsVectorSurface(const std::vector<Handle(Geom_BSplineSurface)>& old_surfaces_vector, SurfaceDirection dir);

    /**
     * Changes the parameter range of the b-spline curve
     */
    asiAlgo_EXPORT static void reparametrizeBSpline(Geom_BSplineCurve& spline, double umin, double umax, double tol=1e-15);

    /**
     * Changes the parameter range of the b-spline surfae
     */
    asiAlgo_EXPORT static void reparametrizeBSpline(Geom_BSplineSurface& spline, double umin, double umax, double vmin, double vmax, double tol);
     ...
     */
    asiAlgo_EXPORT static CTiglApproxResult reparametrizeBSplineNiceKnots(Handle(Geom_BSplineCurve) spline);
     ...
     */
    asiAlgo_EXPORT static CTiglApproxResult reparametrizeBSplineContinuouslyApprox(const Handle(Geom_BSplineCurve) spline, const std::vector<double>& old_parameters,
     ...
     */
    asiAlgo_EXPORT static Handle(Geom_BSplineSurface) flipSurface(const Handle(Geom_BSplineSurface) surface);
     ...
     */
    asiAlgo_EXPORT static Handle(Geom_BSplineSurface) pointsToSurface(const TColgp_Array2OfPnt& points,
     ...
     */
    asiAlgo_EXPORT static std::vector<std::pair<double, double> > intersections(const Handle(Geom_BSplineCurve) spline1, const Handle(Geom_BSplineCurve) spline2, double tolerance=3e-4);
     ...
     *          the scale
     */
    asiAlgo_EXPORT static double scale(const std::vector<Handle(Geom_BSplineCurve)>& splines_vector);
     ...
     *          the scale
     */
    asiAlgo_EXPORT static double scale(const Handle(Geom_BSplineCurve)& spline);

    /// Returns the scale of the point matrix
    asiAlgo_EXPORT static double scale(const TColgp_Array2OfPnt& points);

    /// Returns the scale of the point list by searching for the largest distance between two points
    asiAlgo_EXPORT static double scale(const TColgp_Array1OfPnt& points);

    /**
     * Returns positions, where the curve has kinks (C1 Discontinuities)
     */
    asiAlgo_EXPORT static std::vector<double> getKinkParameters(const Handle(Geom_BSplineCurve)& curve);
     ...
     */
    asiAlgo_EXPORT static SurfaceKinks getKinkParameters(const Handle(Geom_BSplineSurface)& surface);


    /// Checks, whether the point matrix points is closed in u direction
    asiAlgo_EXPORT static bool isUDirClosed(const TColgp_Array2OfPnt& points, double tolerance);

    /// Checks, whether the point matrix points is closed in u direction
    asiAlgo_EXPORT static bool isVDirClosed(const TColgp_Array2OfPnt& points, double tolerance);
     ...
     */
    asiAlgo_EXPORT static std::vector<double> knotsFromCurveParameters(std::vector<double>& params, unsigned int degree, bool closedCurve);

    /// Trims a bspline surface
    asiAlgo_EXPORT static Handle(Geom_BSplineSurface) trimSurface(const Handle(Geom_Surface)& surface, double umin, double umax, double vmin, double vmax);

    /// Trims a bspline curve
    asiAlgo_EXPORT static Handle(Geom_BSplineCurve) trimCurve(const Handle(Geom_BSplineCurve)& curve, double umin, double umax);

    /// Concatenates a list of bspline curves
    asiAlgo_EXPORT static Handle(Geom_BSplineCurve) concatCurves(std::vector<Handle(Geom_BSplineCurve)> curves,
     ...
     */
    asiAlgo_EXPORT static Handle(Geom_BSplineSurface) concatSurfacesUDir(Handle(Geom_BSplineSurface) bspl1, Handle(Geom_BSplineSurface) bspl2, double space_tol=1e-5);
    ...
    */
    asiAlgo_EXPORT static Handle(Geom_BSplineSurface) makeKnotsUniform(Handle(Geom_BSplineSurface) surf, unsigned int nseg_u, unsigned int nseg_v);
};
} // namespace tigl
