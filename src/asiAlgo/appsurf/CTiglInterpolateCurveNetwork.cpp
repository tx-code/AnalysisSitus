    ...
    , m_spatialTol(spatialTol)
{
    ...
    if (profiles.size() < 2) {
        throw Standard_ProgramError("There must be at least two profiles for the curve network interpolation.");
    }

    if (guides.size()  < 2) {
        throw Standard_ProgramError("There must be at least two guides for the curve network interpolation.");
    }
...
//                }
                ...
                throw Standard_ProgramError("U-directional B-spline and v-directional B-spline have more than two intersections with each other!");
            }
        }
    }

    if (fail) {
        throw Standard_ProgramError("U-directional B-spline and v-directional B-spline don't intersect each other!");
    }
}
...
void CTiglInterpolateCurveNetwork::Perform()
{
    ...
    m_plotter.REDRAW_SURFACE("tigl_P1S", m_skinningSurfProfiles, Color_Default);
    m_plotter.REDRAW_SURFACE("tigl_P2S", m_skinningSurfGuides, Color_Default);
    m_plotter.REDRAW_SURFACE("tigl_P12S", m_tensorProdSurf, Color_Default);
...
{
    return CTiglInterpolateCurveNetwork(profiles, guides, tol, nullptr, nullptr).Surface();
}
