    ...
    , m_C2Continuous(continuousIfClosed)
{
    ...
    if (maxDegree < 1) {
        throw Standard_ProgramError("Degree must be larger than 1 in CTiglPointsToBSplineInterpolation!");
    }

    if (points.IsNull()) {
        throw Standard_ProgramError("No points given in CTiglPointsToBSplineInterpolation");
    }

    if (points->Length() < 2) {
        throw Standard_ProgramError("Too few points in CTiglPointsToBSplineInterpolation");
    }
}
    ...
    , m_C2Continuous(continuousIfClosed)
{
    if (static_cast<int>(m_params.size()) != m_pnts->Length()) {
        throw Standard_ProgramError("Number of parameters and points don't match in CTiglPointsToBSplineInterpolation");
    }
