...
void WireGetPointTangent(const TopoDS_Wire& wire, double alpha, gp_Pnt& point, gp_Vec& tangent)
{
    if (alpha < 0.0 || alpha > 1.0) {
        throw Standard_ProgramError("Parameter alpha not in the range 0.0 <= alpha <= 1.0 in WireGetPointTangent");
    }
    // ETA 3D point
    BRepAdaptor_CompCurve aCompoundCurve(wire, Standard_True);
    ...
    Standard_Real len =  GCPnts_AbscissaPoint::Length( aCompoundCurve );
     if (len < Precision::Confusion()) {
         ...
         if (algo.IsDone()) {
             ...
             throw Standard_ProgramError("WireGetPointTangent: Cannot compute point on curve.");
          }
     }
...
{
    if (alpha < 0.0 || alpha > 1.0) {
        throw Standard_ProgramError("Parameter alpha not in the range 0.0 <= alpha <= 1.0 in EdgeGetPointTangent");
    }
    // ETA 3D point
    Standard_Real umin, umax;
    Handle(Geom_Curve) curve = BRep_Tool::Curve(edge, umin, umax);
    ...
    if (len < Precision::Confusion()) {
        throw Standard_ProgramError("EdgeGetPointTangent: Unable to compute tangent on zero length edge");
    }
    ...
    if (algo.IsDone()) {
        ...
        throw Standard_ProgramError("EdgeGetPointTangent: Cannot compute point on curve.");
    }
}
...
gp_Pnt ProjectPointOnShape(const TopoDS_Shape &shape, const gp_Pnt &point, const gp_Vec &direction)
{
    ...
    if (nPoints == 0) {
        throw Standard_ProgramError("Projection of point to shape failed.");
    }
    else if (nPoints > 1) {
        std::cout << "Projection of point of shape has multiple results. Choosing first.";
    }
        ...
        if (faceCurveInter.More()) {
            ...
            while (faceCurveInter.More()) {
                if (!dst.IsEqual(faceCurveInter.Pnt(), Precision::Confusion())) {
                    std::cout << "Multiple Intersections found in GetIntersectionPoint";
                }
                faceCurveInter.Next();
            }
            return true;
...
bool GetIntersectionPoint(const TopoDS_Face& face, const TopoDS_Edge& edge, gp_Pnt& dst, double tolerance)
{
    ...
    if (!intersection) {
        std::cout << "No Intersections found in GetIntersectionPoint";
    }
    return intersection;
}
...
bool GetIntersectionPoint(const TopoDS_Face& face, const TopoDS_Wire& wire, gp_Pnt& dst, double tolerance)
{
    ...
    std::cout << "No Intersections found in GetIntersectionPoint(face, wire)";
    return false;
}

asiAlgo_EXPORT bool GetIntersectionPoint(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2, intersectionPointList& intersectionPoints, const double tolerance)
{
    ...
    while ( EdgeExplorer2.More() ) {
        ...
        while ( EdgeExplorer1.More() ) {
            ...
            if (!Intersector.IsDone()) {
                std::cout << "Error intersecting two wires in GetIntersectionPoint!";
                throw Standard_ProgramError("Error intersecting two wires in GetIntersectionPoint!");
            }
    ...
    if ( intersectionPoints.size() == 0 ) {
        std::cout << "tiglIntersectCurves: The curves do not intersect to the specified tolerance";
        return false;
    }
...
TopoDS_Face GetSingleFace(const TopoDS_Shape& shape)
{
    ...
    if (numFaces < 1) {
        std::cout << "unable to get single face from shape: shape contains no faces";
        throw Standard_ProgramError("unable to get single face from shape: shape contains no faces!");
    }
    else if (numFaces > 1) {
        std::cout << "unable to get single face from shape: shape contains more than one face";
        throw Standard_ProgramError("unable to get single face from shape: shape contains more than one face!");
    }
...
gp_Pnt GetFirstPoint(const TopoDS_Shape& wireOrEdge)
{
    ...
    throw Standard_ProgramError("Shape must be wire or edge");
}
...
TiglContinuity getEdgeContinuity(const TopoDS_Edge& edge1, const TopoDS_Edge& edge2)
{
    ...
    wireMaker.Add(edge1);
    if (!wireMaker.IsDone()) {
        throw Standard_ProgramError("checkEdgeContinuity: Unable to create connected wire");
    }
    wireMaker.Add(edge2);
    if (!wireMaker.IsDone()) {
        throw Standard_ProgramError("checkEdgeContinuity: Unable to create connected wire");
    ...
    wire = wireFixer.Wire();
    if (wire.IsNull()) {
        throw Standard_ProgramError("checkEdgeContinuity: Unable to create common vertex in connected wire");
    }
    ...
    if (edges.Extent() > 2) {
        throw Standard_ProgramError("checkEdgeContinuity: Unexpected error in connected wire");
    }
    ...
    if (commonVertex.IsNull()) {
        throw Standard_ProgramError("checkEdgeContinuity: Unable to find common vertex");
    }
    ...
    if (TopExp::FirstVertex(newedge1).IsSame(commonVertex) && TopExp::LastVertex(newedge2).IsSame(commonVertex)) {
        ...
        throw Standard_ProgramError("checkEdgeContinuity: Unexpected error: Unable to find common vertex in connected wire");
    }
...
TopoDS_Face BuildFace(const TopoDS_Wire& wire)
{
    ...
    std::cout << "Could not build face from wire.";
    throw Standard_ProgramError("BuildFace: Unable to generate face from Wire!");
}
...
//        std::cout << "unable to determine common wire order!";
//        throw Standard_ProgramError("unable to determine common wire order!");
//    }
//    TopTools_SequenceOfShape orderedWireSequence;
//
//    orderedWireSequence = orderWires.Shape();
//    if (orderedWireSequence.Length() != 2) {
//        std::cout << "number of wires in sequence does not match. Expected: 2, Got: " << orderedWireSequence.Length();
//        throw Standard_ProgramError("number of wires in sequence does not match. Expected: 2");
//    }
...
//
//        std::cout << "unable to approximate wires by curves for building face";
//        throw Standard_ProgramError("unable to approximate wires by curves for building face!");
//    }
...
TopoDS_Wire BuildWireFromEdges(const TopoDS_Shape& edges)
{
    ...
    if (wireList.Extent() != 1) {
        std::cout << "Error generating single wire!";
        throw Standard_ProgramError("Error generating single wire in MakeWireFromEdges!");
    }
...
void BuildWiresFromConnectedEdges(const TopoDS_Shape& shape, TopTools_ListOfShape& wireList)
{
    ...
    while (edgeList.Extent() > 0) {
        ...
        if (!wireBuilder.IsDone()) {
            std::cout << "Error creating wire!";
            throw Standard_ProgramError("Error creating wire in BuildWiresFromConnectedEdges!");
        }
...
TopoDS_Wire CloseWire(const TopoDS_Wire& wire)
{
    ...
    if (numEndVertices != 2) {
        std::cout << "invalid number of end vertices found!";
        throw Standard_ProgramError("invalid number of end vertices found in CloseWire!");
    }
    ...
    makeWire.Add(edge);
    if (!makeWire.IsDone()) {
        std::cout << "unable to build closed wire!";
        throw Standard_ProgramError("unable to build closed wire in CloseWire!");
    }
...
TopoDS_Wire CloseWires(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2)
{
    ...
    if (endVertices1.Extent() != 2) {
        std::cout << "Unable to close wires because invalid number of end-vertices found!";
        throw Standard_ProgramError("Unable to close wires because invalid number of end-vertices found!");
    }
...
TopoDS_Wire CloseWires(const TopoDS_Wire& wire1, const TopoDS_Wire& wire2, const gp_Vec& dir)
{
    ...
    if (!makeWire.IsDone()) {
        std::cout << "error during creation of closed wire!";
        throw Standard_ProgramError("error during creation of closed wire in CloseWires!");
    }
...
TopoDS_Shape CutShapes(const TopoDS_Shape& shape1, const TopoDS_Shape& shape2)
{
    ...
    cutter.Build();
    if (!cutter.IsDone()) {
        std::cout << "Error cutting shapes in method CutShapes!";
        throw Standard_ProgramError("Error cutting shapes in method CutShapes!");
    }
    return cutter.Shape();
}
//
...
//            std::cout << ss.str();
//            throw Standard_ProgramError(ss.str());
//        }
//
...
//            if (i < c_tries - 1) {
//                fuzzyValue *= 10;
//                std::cout << "SplitShape failed, retrying with fuzzyValue: " << fuzzyValue;
//                continue;
//            }
//
//            std::ostringstream oss;
//            splitter.GetReport()->Dump(oss);
//            std::cout << "unable to split passed shapes: " << oss.str();
//            throw Standard_ProgramError("unable to split passed shapes: " + oss.str());
//        }
...
//            }
//
//            std::cout << "unable to split passed shapes!";
//            throw Standard_ProgramError("unable to split passed shapes!");
//        }
//#else
//        if (splitter.ErrorStatus() != 0) {
//            std::cout << "unable to split passed shapes!";
...
TopoDS_Face GetNearestFace(const TopoDS_Shape& shape, const gp_Pnt& pnt)
{
    ...
    for (explorer.Init(shape, TopAbs_FACE); explorer.More(); explorer.Next()) {
        ...
        if (!extrema.IsDone() || extrema.NbSolution() < 1) {
            std::cout << "unable to determine nearest point between face and vertex!";
            throw Standard_ProgramError("unable to determine nearest point between face and vertex!");
        }
...
bool IsPointInsideShape(const TopoDS_Shape &solid, gp_Pnt point, Bnd_Box const* bounding_box)
{
    ...
    if (solid.ShapeType() != TopAbs_SOLID) {
        throw Standard_ProgramError("The shape is not a solid");
    }
...
//    default:
//        throw Standard_ProgramError("Invalid coordinate system");
//    }
//}
//
